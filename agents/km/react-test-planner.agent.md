...
tools:
  ...
  - read_file
  - run_command
  - react-test/planner_read_config
  - react-test/planner_list_tests
  - react-test/planner_run_coverage
  - react-test/planner_analyze_source
...
### 6. Save the Plan
Submit the complete test plan using `planner_save_plan` as a markdown file under `specs/` with a clear filename matching the target module.
