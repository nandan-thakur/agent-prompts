...
description: Sub-agent for test coverage gaps, Redux testing, TypeScript type safety, and code maintainability. Invoked by Code Review orchestrator.
model: gpt-5-mini
...
**Redux Testing:**
- Reducers tested with raw action objects instead of using action creators
- Missing tests for async thunks (pending/fulfilled/rejected states)
- Selector tests missing — especially `createSelector` derived state
- Store not properly configured in test setup (missing middleware)
- Missing integration tests for connected components with real Redux store
