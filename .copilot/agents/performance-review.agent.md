...
description: Sub-agent for React render performance, re-renders, Redux selectors, bundle size, and runtime optimization. Invoked by Code Review orchestrator.
model: gpt-5-mini
...
**Redux Performance:**
- `useSelector` returning new references (objects/arrays) — use `createSelector`
- Selectors doing expensive computation without memoization
- Dispatching actions in loops — should batch with `batch()` or `unstable_batchedUpdates`
- Overly broad store subscriptions — component subscribes to entire slice
- Unnecessary `useDispatch` + `useEffect` chains — use middleware instead
